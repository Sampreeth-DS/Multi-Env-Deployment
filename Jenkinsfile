    ...
    stages {
        ...
        stage('Deploying Application in DEV env') {
            steps {
                script {
                    try {
                        sh "helm upgrade --install python-app $HELM_CHART_PATH -n dev -f $HELM_CHART_PATH/dev_values.yaml --set image.tag=$NEW_VERSION revisionHistoryLimit=3"
                    }
                    catch (Exception e) {
        ...
        stage('Deploying Application in TEST env') {
            steps {
                script {
                    try {
                        sh "helm upgrade --install python-app $HELM_CHART_PATH -n test -f $HELM_CHART_PATH/test_values.yaml --set image.tag=$NEW_VERSION revisionHistoryLimit=3"
                    }
                    catch (Exception e) {
        ...
        stage('Deploying Application in PROD env') {
            steps {
                script {
                    try {
                        sh "helm upgrade --install python-app $HELM_CHART_PATH -n prod -f $HELM_CHART_PATH/prod_values.yaml --set image.tag=$NEW_VERSION revisionHistoryLimit=3"
                    }
                    catch (Exception e) {
